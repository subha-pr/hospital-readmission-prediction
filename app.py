...
def predict_value(filename):
	df_s = pd.read_csv(filename,index_col=False)
	if(df_s.discharge_disposition_id[0] in [11,13,14,19,20,21]):
		return 0
	dfk=preprocess(df_s)
	...
	logging.info(pred1,pred2,pred3,pred4)
...
def prediction(filename):
	result = predict_value(os.path.join('uploads', filename))
	if(result<2):
		prediction = "Patient does not need to be readmitted"
	else:
		prediction = "Patient needs to be readmitted"
